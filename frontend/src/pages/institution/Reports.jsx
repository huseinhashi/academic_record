...
import { Calendar as CalendarIcon, FileText, FileDown, BarChart3, Users, GraduationCap, CheckCircle } from "lucide-react";
...
export const InstitutionReports = () => {
  ...
  const fetchReportData = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/reports/institution/${reportType}`, {
        params: {
  ...
  const fetchSummaryStats = async () => {
    try {
      const response = await api.get(`/reports/institution/summary`, {
        params: {
          startDate: format(dateRange.from, 'yyyy-MM-dd'),
          endDate: format(dateRange.to, 'yyyy-MM-dd')
        }
  ...
  const exportToPDF = () => {
    ...
    const reportTypeLabel = reportTypes.find(r => r.value === reportType)?.label || "Institution Report";
    doc.text(reportTypeLabel.toUpperCase(), pageWidth / 2, 18, { align: 'center' });
    
    // Subtitle
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text('Institution Academic Management Report', pageWidth / 2, 28, { align: 'center' });
    
    // Institution Information Card
    doc.setFillColor(...lightGray);
    doc.roundedRect(14, 50, pageWidth - 28, 35, 3, 3, 'F');
    
    // Institution info content
    doc.setTextColor(...secondaryColor);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('INSTITUTION INFORMATION', 20, 62);
    
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    doc.text(`Institution: ${user.name}`, 20, 72);
    ...
    if (finalY < pageHeight - 50) {
      ...
      doc.text('This report was generated automatically and contains institution academic data.', 20, finalY + 32);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Institution Reports</h1>
        <p className="text-muted-foreground">
          Generate and export institution-specific reports and analytics
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summaryStats.totalStudents || 0}</div>
            <p className="text-xs text-muted-foreground">
              {summaryStats.newStudents || 0} new this period
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {summaryStats.pendingRecords || 0} pending verification
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summaryStats.verificationRate || 0}%</div>
            <p className="text-xs text-muted-foreground">
              {summaryStats.verifiedRecords || 0} records verified
            </p>
          </CardContent>
        </Card>
